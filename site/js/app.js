...
// ===== API CONFIG =====
var FC_API_HEADERS = { 'X-API-Key': 'fcweb_RETIRED_KEY_ROTATED_20260514' };
...
(function(){
    ...
    function fetchChampion(){
        fcFetch('/api/swarm_champion').then(function(r){ return r.json(); }).then(function(c){
            ...
            if (c.net_pnl !== undefined) html += '<div style="background:var(--bg2,#0a0f18);border:1px solid var(--border);border-radius:6px;padding:10px 12px;text-align:center;"><div style="font-size:1.1rem;font-weight:800;color:' + ((c.net_pnl || 0) >= 0 ? 'var(--green,#4ecdc4)' : 'var(--red,#ef4444)') + ';">$' + Number(c.net_pnl || 0).toLocaleString('en-US', {maximumFractionDigits: 2}) + '</div><div style="font-size:0.55rem;text-transform:uppercase;letter-spacing:1px;color:var(--text-dim,#5a6a82);margin-top:2px;">Backtest PnL</div></div>';
